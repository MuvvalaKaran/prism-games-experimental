...
all_fields_details = [\
    ...
    {'name': 'Transitions', 'descr': 'Number of Transitions in model', 'regexp': 'Transitions: *(.+)'}, \
    {'name': 'Choices', 'descr': 'Number of Choices in model', 'regexp': 'Choices: *(.+)'}, \
    {'name': 'Max/Avg', 'descr': 'Number of Transitions in model', 'regexp': 'Max/avg: *(.+)'}, \
    ...
    {'name': 'prob0_check_iters', 'descr': 'Prob0 model check for property', 'type': 'string', 'regexp': 'Prob0 \(maxmin\) took ([^ \n]+) iterations and *(.+) seconds'}, \
    {'name': 'prob1_check_iters', 'descr': 'Prob1 model check for property', 'type': 'string', 'regexp': 'Prob1 \(maxmin\) took ([^ \n]+) iterations and *(.+) seconds'}, \
    # {'name': 'prob0_check_time', 'descr': 'Prob0 model check time for property', 'type': 'string', 'regexp': 'Prob0 \(maxmin\) took ([^ \n]+) iterations and *(.+) seconds', 'match': 'second'}, \
    # {'name': 'prob1_check_time', 'descr': 'Prob1 model check time for property', 'type': 'string', 'regexp': 'Prob1 \(maxmin\) took ([^ \n]+) iterations and *(.+) seconds'}, \
    {'name': 'Init Value', 'descr': 'Value Initial state', 'type': 'string', 'regexp': 'Value in the initial state: *(.+)'},\
...
def grep_for_info_file(logFile, fields):
    ...
    for line in open(logFile, 'r').readlines():
        ...
        if line_num == 1:
            ...
            # I am skipping program name - Dont need it.
            pass
        ...
        for field in fields:
            ...
            elif 'regexp' in field_details and (info[field] == '' or ('match' in field_details and field_details['match'] == 'second')):
                regexp = field_details['regexp']
                print(regexp)
                print(line)
                m = re.search(regexp, line)
                # if not m in None:
                    # seconds_values = m.group(2)
